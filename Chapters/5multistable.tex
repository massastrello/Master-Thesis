...
%
%\iffalse
%Note that the damping injection control action $v = -k_dy$ does not change the location of the fixed points of the system. 
%%{\color{red}
...
\begin{figure}[!h]
	\centering
	\input{Drawings/interconnection.tex}
	%
...
To complete the model, let us define the \textit{auxiliary hybrid system} which formalize the behavior of $\tau$ and $r$:
%
\begin{equation}
	\left\{
	%
...
The hybrid port--Hamiltonian system defined above is not passive. In fact, there is no guarantees that
%
\begin{equation}
...
%%
%\begin{equation}
%	\|\bm\nu(r)\|_2<\infty ~\Rightarrow~ \forall\xb\in\X\quad\exists \eta\in\R~:~\eta = \sup_{\bm\nu}\Ha(\xb+\Bb\bm\nu)
%\end{equation}
%%

%
%\begin{prop}
%	Consider an LTI system subjected to MES--HMS control. It holds
%	%
%	\begin{equation}
%		
%	\end{equation}
...
% \end{equation}
% %
% By introducing a dummy variable $s$, representing the logic state of the system, i.e. $s = i$ in $S_i$, and performing a state extension:
% %
% \begin{equation}
